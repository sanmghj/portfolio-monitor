import type { AIPrompt, HoldingInsights, NewsItem, Portfolio, PriceData, Stock, User } from '../types';
...
function mapPortfolio(dto: ApiPortfolio): Portfolio {
  return {
    ...
    monthlyBudget: dto.monthly_budget ?? null,
    targetWeight: dto.target_weight ?? null,
    stockCount: dto.stock_count ?? 0,
    totalValue: dto.total_value ?? 0,
    profit: dto.pnl ?? 0,
    profitRate: dto.pnl_percent ?? 0,
  };
}
...
function mapStock(dto: ApiHolding): Stock {
  return {
    ...
    purchasePrice: Number(dto.avg_price),
    currentPrice: Number(dto.current_price ?? dto.avg_price),
...
export const getHoldingInsights = async (stockId: string): Promise<HoldingInsights> => {
  ...
  return {
    latestPrice: data.latest_price ? Number(data.latest_price.price) : null,
...
export const getPriceHistory = async (stockId: string): Promise<PriceData[]> => {
  const user = getCurrentUser();
  if (!user) return [];
  const history = await api<Array<{ date: string; price: number; volume: number }>>(
